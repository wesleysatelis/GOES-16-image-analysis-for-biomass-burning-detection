...
for i in entrys:
	...
	band2 = np.genfromtxt(path + i + ".C02.txt", delimiter=',')
	band3 = np.genfromtxt(path + i + ".C03.txt", delimiter=',')
	ndvi = np.divide(np.subtract(band3,band2), np.add(band3, band2))
	# bri = 1/((0.1 - band2)^2 + (0.06 - band3)^2)
	# ndvi = np.flipud(ndvi)
	# print(i)
	# fig = plt.imshow(ndvi)
	# plt.axis('off')
	...
	# netcdf info
	# OrderedDict([('MINLON', -53.5), ('MAXLON', -44.0), ('MINLAT', -25.5), ('MAXLAT', -19.5), ('DATA_MINLON', -53.5), 
	# ('DATA_MAXLON', -44.0), ('DATA_MINLAT', -25.5), ('DATA_MAXLAT', -19.5), ('COVER', 100.0), ('RESOLUTION', 0.005), 
	# ('NLON', 2109), ('NLAT', 1332), ('BPP', 2), ('NCHAN', 4), ('DATETIME', 'ء�'), ('PRODUCT', ''), ('SETUP', ''), 
	# ('SATELLITE', 'ء�'), ('CHANNELS', ' C01,C02,C03,C05'), ('SEPARATOR', 'h�\x04\x08X���\\j�')])
	...
	fig.axes.get_xaxis().set_visible(False)
	fig.axes.get_yaxis().set_visible(False)
	plt.savefig("/run/media/wesley/6CD80ADD0368A759/Cepagri/ndvi/all/" + i[0:-4] + '.png', dpi=DPI, bbox_inches='tight', pad_inches=0)
	print(i)
	ax.clear()
	plt.close(ax)
